...
function onEditInstallable(e) {
  ...
  try {
    ...
    const statusKey = statusNow.toLowerCase();
    ...
    const statusToWebhook = {
      ...
      "nova versão": N8N.WEBHOOK_NOVA_VERSAO,
    ...
    if (webhookUrl && (isDataComplete || statusKey !== "novo")) {
      ...
      if (isManualStatusChange || acquireRowLock_(row, updAt)) {
        console.log(`🚀 ENVIANDO (${statusNow}): ID=${payload.codigo_prop}`);
        postToN8N_(payload, webhookUrl);
      }
    }
...
function postToN8N_(payload, url) {
  ...
  try {
    ...
    console.log(`Enviado para N8N: ${url}`);
  } catch (e) {
...
function acquireRowLock_(row, updatedAtMs) {
  ...
  const now = Date.now();
  const until = Number(sh.getRange(row, colUntil).getValue() || 0);
  if (now < until) {
    if (!(updatedAtMs && updatedAtMs > until)) return false;
  }
...
function applyValidations_() {
  ...
  try {
    sh.getRange(
      ...
      lastRow - 1,
    ).setDataValidation(dvList(CFG.CONTATOS_COMERCIAIS));
  } catch (_) {}
...
function leads_setup() {
  ...
  if (triggers.length === 0) FIX_TRIGGERS_NOW();
  SpreadsheetApp.flush();
